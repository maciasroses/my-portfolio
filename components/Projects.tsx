            ...
            <Carousel
                ...
                prevArrow={({ handlePrev }) => (
                    <IconButton
                        ...
                        color="white"
                        onClick={handlePrev}
                        className="!absolute top-2/4 left-4 -translate-y-2/4 z-20 bg-black/50 hover:border-2 hover:border-white"
                    >
                ...
                nextArrow={({ handleNext }) => (
                    <IconButton
                        ...
                        color="white"
                        onClick={handleNext}
                        className="!absolute top-2/4 !right-4 -translate-y-2/4 z-20 bg-black/50 hover:border-2 hover:border-white"
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-3 h-3 sm:w-5 sm:h-5">
                    ...
                    <div key={project.id} className='relative w-full h-full flex justify-center items-center' onMouseEnter={() => setIsTextVisible(true)} onMouseLeave={() => setIsTextVisible(false)}>
                        ...
                        {isTextVisible && (
                            <div className='w-full h-full overflow-y-scroll overflow-x-hidden py-5 px-20 lg:p-10 absolute top-0 flex flex-col justify-start lg:justify-end items-center bg-black/50 animate-fade' onClick={() => setIsTextVisible(false)}>
                                <p className='z-10 text-xl md:text-2xl lg:text-4xl text-white'>{project.title}</p>
                                <p className='z-10 text-base md:text-lg lg:text-2xl text-gray-300'>{t('lan') === 'en' ? project.description : project.description_es}</p>
                                <Link href="/" className='z-10 rounded-xl px-4 py-1 text-white border border-black bg-black hover:border-white'>More</Link>
                            </div>
                        )}
                    </div>
                ))}
